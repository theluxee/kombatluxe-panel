...
while(Sair == False):
    ...
    for letter in "K O M B A T L U X E ":
    ...
    print(f"[{C}{R}+{C}] Licenciado pelo TheLuxe NoPainNOGain. Não me responsabilizo por qualquer uso indevido.")
    time.sleep(2)
    tools.clear()
    time.sleep(2)
    print(f"{C}[{G}Dev's{C}] Coded By: {CY} THELUXE {CO} and {CY} NOPAINNOGAIN {CO} in 07/02/2021")
    print()
    print(f'{C}{G}{result}{C}')    
    ...
    if anim == 1:
        time.sleep(1)
    print(f'{C}[{Y}*{C}]Meu Pix: {CY}d959c9dc-083d-4390-93cc-35e437d2595a{CO}')
    print(f'{C}[{Y}*{C}]Patrocinadores: {CY}THELUXEE{CO}, {CY}NOPAINNOGAIN{CO}, {CY}ᬊTELUXE ↯ NO PAIN↯{CO} e {CY}N 3 X 0{CO}')
    print(f'[{C}{R}+{C}] Se vierem no meu PV perguntar quando as consultas vão voltar Ou Atualizar, vou dar block ou ignorar ass THELUXE - NOPAINNOGAIN.')
    print()
    ...
    {C}{G}[89]{C} THELUXE O BRABO[1] 
    
    {C}{G}[90]{C} THELUXE O BRABO[2] 
    
    {C}{G}[92]{C} DISCORD KOMBAT LUXE
    
    {C}{G}[93]{C} Meu canal TheLuxe Nogocios
    ...
    print()
    print(f"{C_GREY89}{C_RED1}ESTE PAINEL FOI DISPONIBILIZADO POR UM PRECO SIMBOLICO, SE VOCÊ PAGOU MUITO CARO POR ISSO FOI ENGANADO ASS THELUXE - NOPAINNOGAIN.{NO_FORMAT}")
